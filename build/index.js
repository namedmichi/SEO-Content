  ...
  edit: props => {
    ...
    async function gpt_call() {
      ...
      if (!premium) {
        ...
        }).then(response => response.json()).then(data => result = data.choices[0]['message']['content'].split('\n').slice(1).slice(1).join('\n')).then(data => console.log(result)).then(data => {
...
async function gpt_make_request(content, props, prompt) {
  ...
  if (!premium) {
    try {
      ...
      alert('Es ist ein Fehler aufgetreten. Bitte warten Sie ein paar Sekunden und versuchen es dann erneut. Bei weiteren Probleme kontaktieren Sie bitte den Support');
      throw error;
    }
  } else {
  ...
  if ((0,_wordpress_data__WEBPACK_IMPORTED_MODULE_4__.select)('core/block-editor').getBlock(blockId).attributes.content != undefined) {
    (0,_wordpress_data__WEBPACK_IMPORTED_MODULE_4__.select)('core/block-editor').getBlock(blockId).attributes.content = contentText.trim().replace(/^"(.*)"$/, '$1');
  } else {
    (0,_wordpress_data__WEBPACK_IMPORTED_MODULE_4__.select)('core/block-editor').getBlock(blockId).attributes.text = contentText.trim().replace(/^"(.*)"$/, '$1');
  }
  var updatedAttributes = (0,_wordpress_data__WEBPACK_IMPORTED_MODULE_4__.select)('core/block-editor').getBlock(blockId).attributes;
